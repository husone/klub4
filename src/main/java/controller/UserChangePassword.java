...
public class UserChangePassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String op = request.getParameter("user-oldPass");
        String np = request.getParameter("user-newPass");
        HttpSession session = request.getSession();
        User userData = (User) session.getAttribute("userData");
        if ((BCrypt.checkpw(op, userData.getPassword()))) {
            UserDAO.updatePW(userData.getUserID(), np);
            request.setAttribute("success", "Update successfully");
        } else {
            request.setAttribute("error", "Update failed: Wrong password");
        }
